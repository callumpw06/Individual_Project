...
stability_status = zeros(length(t), 1); 
...
%----- 3. COMPACT PLOTTING (Using TiledLayout) -----
figure('Color', 'w', 'Position', [50 50 1200 400], 'Theme', 'light');

% Create a 2x2 grid with tight spacing
...
yline(s_v, 'r--', 'Mach 1');
ylabel('Speed / Mach', 'Interpreter', 'latex'); 
title('1. Breaking Sound Barrier', 'Interpreter', 'latex');
set(gca,'FontSize',18);
legend('Speed', 'Mach', 'Location', 'NorthWest', 'Interpreter', 'latex', 'FontSize', 12); 
...
yline(pitch_attitude, 'k:', 'Pilot Input, $\alpha$', 'Interpreter', 'latex');
ylabel('Angle (rad)', 'Interpreter', 'latex'); 
title('2. Flight Path Stability', 'Interpreter', 'latex');
set(gca,'FontSize',18);
legend('\theta (Flight Path)', '\alpha-\theta', 'Location', 'Best', 'Interpreter', 'latex', 'FontSize', 12);
ylim([-0.13 0.16]);
...
title('3. Trajectory', 'Interpreter', 'latex');
set(gca,'FontSize',18);
...
text(1, 0.05, 'UNSTABLE (max(Re($\lambda_i$)) > 0)', 'Color', 'r', 'FontWeight', 'bold', 'Interpreter', 'latex');
text(1, -0.05, 'STABLE (max(Re($\lambda_i$)) < 0)', 'Color', 'g', 'FontWeight', 'bold', 'Interpreter', 'latex');
xlim([0 10]); ylim([-0.1 0.1]);
xlabel('Time (s)', 'Interpreter', 'latex'); 
ylabel('max(Re($\lambda$))', 'Interpreter', 'latex');
title('4. Lyapunov Stability Analysis', 'Interpreter', 'latex');
set(gca,'FontSize',18);

% Add a main title for the dashboard
title(t_layout, 'Supersonic Flight: Stability Analysis', 'FontSize', 18, 'FontWeight', 'bold', 'Interpreter', 'latex');
